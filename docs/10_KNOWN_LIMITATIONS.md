...
| 0005 | 入力デバイスの誤選択に気付けない | **未対応**（改善提案） | 録音内容そのもの。気付かないまま長時間の録音が無価値になる。0019 の入力レベルメーター・低音量警告・入力テストで一部緩和 |
...
| 0019 | 入力音量の自動補正・事前テスト・低音量検知 | 解決（#0004 を統合） |
| 0004 | `session.json` の `segments_path` が指すファイルが生成されない | **#0019 へ統合して解決**（realtime でも実際に書き出す） |
...
- **自動補正には上限がある。** 最大 +24 dB。発話レベルが -48 dBFS を大きく下回る
  （例: マイクから 10m 以上、または極端に小さい声）と、補正しても
  文字起こしに必要なレベルへ届かない。入力テストは `入力が小さすぎる` と判定する
- **SNR は補正で改善しない。** ゲインは発話と環境音を同じだけ持ち上げる。
  騒がしい部屋では、マイクを話者へ近づける以外に手がない
- **既にクリップした音声は復元できない。** 入力段で音割れした録音は、
  `recording.wav` の時点で情報が失われている。クリッピング防止は
  「補正でクリップさせない」ためのもので、入力そのものの音割れは防げない
- 補正は 1 フレーム（128ms）遅れて反映される。先読みを持たない代わりに
  不連続が生じない設計であり、会議音声では実用上問題にならない
- ノイズフロアの推定は非対称追従のため、真値よりやや高めに出る
  （実測 -57.2 dBFS に対し、オフライン解析の 10%ile は -61.8 dBFS）。
  発話判定は必ず下回るため実害は無いが、診断ログの数値をオフライン解析と
  直接比較するときは差を踏まえること
- 入力テストの「推定通過率」は、発話レベルの中央値と下位 20% から線形に
  近似した**概算**。実際の文字起こし成功率そのものではない
- **最遠席・低音量環境での実測は未確認**。2026-09-14 の実機検証（55.5 分 /
  14,996 字 / レベル起因のスキップ 0%）は入力レベルが高い条件（1 分窓 RMS
  中央値 -33.2 dBFS）で行われており、0019 が対象とする遠方・低音量
  （-48 dBFS 前後）での効果は分離できていない。
  再現条件と確認項目は `docs/issues/0019` の「将来の確認事項」を参照
